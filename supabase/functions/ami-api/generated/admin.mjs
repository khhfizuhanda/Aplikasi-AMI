...
const MASTER_ENUMS = Object.freeze({
  PRODI: {Jenjang:['S1','S2','S3']},
  UNIT: {JenisUnit:['BIRO','LEMBAGA','UPT','PUSAT','DIREKTORAT','FAKULTAS','PASCASARJANA','UNIT LAINNYA']},
  AUDITOR: {Sertifikasi:['IYA','TIDAK']},
  PIMPINAN: {Level:['YAYASAN','UNIVERSITAS','FAKULTAS','PRODI','UNIT']}
});
function parseBoolean(value, field = 'Active', defaultValue = true) {
  if (value === undefined || value === null || clean(value) === '') return defaultValue;
  if (value === true || ['TRUE','1','YES','YA','IYA','AKTIF'].includes(upper(value))) return true;
  if (value === false || ['FALSE','0','NO','TIDAK','TIDAK AKTIF','NONAKTIF'].includes(upper(value))) return false;
  throw new Error(`${field} harus IYA/TIDAK.`);
}
function validateMasterInput(type, input, strict = false) {
  const key = upper(type).replace(/^MASTER_/, '');
  const required = {PRODI:'NamaProdi', UNIT:'NamaUnit', AUDITOR:'Nama', PIMPINAN:'Nama'}[key];
  if (!clean(input && input[required])) throw new Error(`${required} wajib diisi.`);
  for (const [field, values] of Object.entries(MASTER_ENUMS[key] || {})) {
    const value = upper(input && input[field]);
    if (!value && strict) throw new Error(`${field} wajib diisi.`);
    if (value && !values.includes(value)) throw new Error(`${field} harus ${values.join('/')}.`);
  }
  parseBoolean(input && input.Active);
  return key;
}
...
async function saveMaster(pool, schema, user, type, input) {
  ...
  return inTransaction(pool, async db => {
    input = input || {}; validateMasterInput(type, input); let keyValue = clean(input[def.key]);
    ...
    const stamp = now();
    const record = {...fieldsFrom(input, def.fields), [def.key]:keyValue, Active: parseBoolean(input.Active) ? 'true' : 'false', UpdatedAt:stamp};
...
async function bulkImportMaster(pool, schema, user, type, rawText) {
  requireAdmin(user); const key = upper(type).replace(/^MASTER_/, ''); if (!masterDefs[key]) throw new Error('Jenis master tidak dikenali.');
  return inTransaction(pool, async db => {
    let inserted = 0, updated = 0, skipped = 0; const errors = []; const seen = new Set();
    const lines = String(rawText || '').split(/\r?\n/).filter(line => clean(line));
    for (const [lineIndex, line] of lines.entries()) {
      const c = line.split('\t');
      if (['KODEPRODI','KODEUNIT','NIDN_NIK','NIDN/NIK','NAMA'].includes(upper(c[0]))) continue;
      await db.query('SAVEPOINT bulk_row');
      try {
        let record;
        ...
        validateMasterInput(key, record, true);
        const identity = key === 'PRODI' ? `${norm(record.KodeProdi)}|${norm(record.NamaProdi)}` : key === 'UNIT' ? `${norm(record.KodeUnit)}|${norm(record.NamaUnit)}` : key === 'AUDITOR' ? `${norm(record.NIDN_NIK)}|${norm(record.Nama)}|${norm(record.Unit)}` : `${norm(record.Nama)}|${norm(record.Jabatan)}`;
        if (seen.has(identity)) throw new Error('Data master duplikat pada input yang sama.');
        seen.add(identity);
        const def = masterDefs[key]; const existing = (await all(db, schema, def.table)).find(row => key === 'PRODI' ? (norm(row.KodeProdi) === norm(record.KodeProdi) || norm(row.NamaProdi) === norm(record.NamaProdi)) : key === 'UNIT' ? (norm(row.KodeUnit) === norm(record.KodeUnit) || norm(row.NamaUnit) === norm(record.NamaUnit)) : key === 'AUDITOR' ? ((clean(record.NIDN_NIK) && norm(row.NIDN_NIK) === norm(record.NIDN_NIK)) || (norm(row.Nama) === norm(record.Nama) && norm(row.Unit) === norm(record.Unit))) : (norm(row.Nama) === norm(record.Nama) && norm(row.Jabatan) === norm(record.Jabatan)));
        ...
        await db.query('RELEASE SAVEPOINT bulk_row');
      } catch (error) { await db.query('ROLLBACK TO SAVEPOINT bulk_row'); skipped++; if (errors.length < 50) errors.push(`Baris ${lineIndex + 1}: ${error.message}`); }
    }
...
function normalizeStandard(input, old) {
  ...
  record.StandardFamilyID = clean(input.StandardFamilyID) || clean(old && old.StandardFamilyID) || id('STDFAM'); record.Versi = clean(input.Versi) || clean(old && old.Versi) || '1.0'; record.TahunBerlakuMulai = clean(input.TahunBerlakuMulai) || clean(old && old.TahunBerlakuMulai) || record.TahunSumber; record.TahunBerlakuSampai = clean(input.TahunBerlakuSampai); const status = upper(input.StatusStandar || (old && old.StatusStandar) || 'DRAFT'); if (!['DRAFT','BERLAKU','DICABUT','ARSIP'].includes(status)) throw new Error('StatusStandar harus DRAFT/BERLAKU/DICABUT/ARSIP.'); record.StatusStandar = status; if (record.StatusStandar === 'BERLAKU' && !record.TahunBerlakuMulai) throw new Error('Tahun Berlaku Mulai wajib diisi untuk standar berstatus BERLAKU.'); record.ReplacesStandardID = clean(input.ReplacesStandardID) || clean(old && old.ReplacesStandardID); record.Origin = clean(input.Origin) || clean(old && old.Origin) || 'MANUAL'; record.Notes = clean(input.Notes); record.Active = record.StatusStandar === 'BERLAKU' ? 'true' : 'false'; record.SourceHash = standardHash(record); return record;
}
...
async function saveTeams(pool, schema, user, items, publish) {
  requireAdmin(user);
  if (!Array.isArray(items) || !items.length) throw new Error('Pilih minimal satu auditi.');
  return inTransaction(pool, async db => {
    const audits = await all(db, schema, 'AMI_AUDITI');
    const auditors = await all(db, schema, 'MASTER_AUDITOR');
    const teams = await all(db, schema, 'AMI_TEAM');
    const assignments = await all(db, schema, 'AMI_STANDARD_ASSIGN');
    const seen = new Set();
    for (const item of items) {
      const auditId = clean(item.AuditID);
      const target = audits.find(row => clean(row.AuditID) === auditId);
      if (!target || upper(target.Status) !== 'DRAFT BPM' || await assignmentStarted(db, schema, auditId)) throw new Error('Tim hanya dapat diubah pada draft yang belum mulai diisi.');
      if (seen.has(auditId)) throw new Error('Audit duplikat pada batch tim.');
      seen.add(auditId);
      const ids = [item.LeadAuditorID, item.Member1ID, item.Member2ID].map(clean);
      if (ids.some(value => !value) || new Set(ids).size !== 3) throw new Error('Pilih satu Lead dan dua anggota yang berbeda.');
      if (ids.some(value => !auditors.some(row => clean(row.AuditorID) === value && truthy(row.Active)))) throw new Error('Auditor tidak ditemukan atau tidak aktif.');
      if (publish && !assignments.some(row => clean(row.AuditID) === auditId && truthy(row.Active))) throw new Error('Standar audit belum ditetapkan.');
    }
    for (const item of items) {
      const auditId = clean(item.AuditID), old = teams.find(row => clean(row.AuditID) === auditId);
      const record = {TeamID:old?.TeamID || id('TEAM'), AuditID:auditId, LeadAuditorID:clean(item.LeadAuditorID), Member1ID:clean(item.Member1ID), Member2ID:clean(item.Member2ID), AssignedAt:old?.AssignedAt || now(), AssignedBy:user.Nama, UpdatedAt:now()};
      if (old) await update(db, schema, 'AMI_TEAM', 'TeamID', old.TeamID, record); else await insert(db, schema, 'AMI_TEAM', record);
      if (publish) await update(db, schema, 'AMI_AUDITI', 'AuditID', auditId, {Status:'PENUGASAN DITERBITKAN', PublishedAt:now()});
      await audit(db, schema, user, publish ? 'PUBLISH_ASSIGNMENT' : 'SAVE_TEAM', 'AMI_TEAM', auditId, old, record);
    }
    return {ok:true, saved:items.length, published:publish ? items.length : 0, message:publish ? 'Tim tersimpan dan penugasan diterbitkan.' : 'Tim auditor tersimpan.'};
  });
}
const saveAuditTeamsBatch = (pool, schema, user, items) => saveTeams(pool, schema, user, items, false);
const saveTeamsAndPublishBatch = (pool, schema, user, items) => saveTeams(pool, schema, user, items, true);

export {saveAuditTeamsBatch,saveTeamsAndPublishBatch,saveCycle,setActiveCycle,saveMaster,bulkImportMaster,saveStandard,createStandardVersion,deleteStandardDraft,bulkImportStandards,createAuditiFromMaster,assignStandards,reopenAuditAssignment,resetUserPassword,resetUserPasswordsBulk,syncUsersFromMasters,requireAdmin,hashPassword};
